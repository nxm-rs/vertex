...
- The peer stack (`vertex-swarm-peer`, `vertex-swarm-peer-score`, `vertex-swarm-peer-manager` plus their `vertex-net-peer-*` and `vertex-net-local` deps) builds for `wasm32-unknown-unknown` and CI enforces it (the `wasm` job in `.github/workflows/unit.yml`). The wasm cone is single-threaded, so it builds on the pinned stable toolchain: no nightly, no `atomics` target feature, no `build-std`. The only wasm rustflag is the `getrandom_backend="wasm_js"` cfg in `.cargo/config.toml` under `[target.wasm32-unknown-unknown]`. Run it with `cargo build --target wasm32-unknown-unknown -p vertex-util-runtime -p vertex-swarm-peer-score -p vertex-swarm-peer-manager -p vertex-swarm-identity`.
...
- `vertex-swarm-accounting-core`, `vertex-swarm-accounting-pricing`, `vertex-swarm-accounting-pseudosettle` (accounting logic; no IO).
...
- `crates/swarm/stream` (`vertex-swarm-stream`): the transport-agnostic byte-bounded bulk get/put combinator over the `SwarmChunkProvider`/`SwarmChunkSender` traits. It depends only on `vertex-swarm-api`, `nectar-primitives`, `vertex-tasks`, and `futures`, so it is light on both the wasm and FFI cones. Its `src/wasm.rs` module (`cfg(target_arch = "wasm32")`) is the browser adapter that surfaces the streams to JS as async iterators; the wasm-only `wasm-bindgen`/`wasm-bindgen-futures`/`js-sys` deps sit under the crate's `cfg(target_arch = "wasm32")` target table. The native FFI adapter in `vertex-ffi` wraps the same core.
- All `nectar-*` deps.
...
- Two clocks rule: `vertex_tasks::time` is the runtime clock home. Its `Instant` is the timer clock (tokio's pause-aware clock on native, the browser clock on wasm) and its `now_unix_*` helpers are the runtime wall clock (derived from the timer clock on native, the browser clock on wasm); internal bookkeeping and persistence timestamps read there so paused-clock tests and the sim advance them with the timers. `vertex_util_runtime::time::{Instant, SystemTime, now_unix_*}` are the platform clock, always real: wire-visible timestamps, log and metric stamps, and any comparison against a remote-produced timestamp.
...
- `trunk build --release` (stable toolchain) produces `dist/` with the wasm module, the wasm-bindgen glue, the HTML shell, and the stylesheet.
- The demo is single-threaded, so it needs no special linker recipe: prebuilt std, ordinary linear memory, and the `getrandom_backend="wasm_js"` cfg in `bin/swarm-demo/.cargo/config.toml`. `wasm-bindgen-futures` runs its task queue on the normal Promise microtask scheduler, so no `SharedArrayBuffer` and no cross-origin isolation (COOP/COEP) are required.
