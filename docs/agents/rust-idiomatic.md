...
- `proptest` for codec round-trips and any validation function with a non-trivial input space. The shared generator layer is the workspace-wide `arbitrary` feature: valid-by-construction `Arbitrary` impls on the owning crates (`vertex-swarm-primitives` for the proximity-role and chunk wrappers, `vertex-swarm-peer` for really signed peer records under `ARBITRARY_NETWORK_ID`, per-protocol codec crates such as `vertex-swarm-net-pushsync` for their wire types), reusing nectar's `arbitrary` impls and `nectar_primitives::generators`/`nectar_postage::generators` for nectar-owned values rather than reimplementing them. Bridge to proptest with `proptest_arbitrary_interop::arb::<T>()`, or map a byte strategy through `arbitrary::Unstructured` into a generator; libFuzzer round-trip targets under `fuzz/` consume the same impls, so both drive one construction path. Enable these features from dev-dependencies only; they stay out of default features and every shipped or wasm cone. The `vertex-swarm-test-utils` `proptest` feature (`strategies` module) carries thin `arb`-based wrappers; `vertex-net-codec::prop_assert_proto_roundtrip!` is the round-trip helper for `ProtoMessage` codecs. Every wire protocol codec carries a `proptests` module; `crates/swarm/net/pullsync` (generator bridge) and `crates/swarm/net/pushsync` (crate `arbitrary` feature shared with the fuzz targets) are the reference adopters.
