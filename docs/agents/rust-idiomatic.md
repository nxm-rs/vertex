...
- `proptest` for codec round-trips and any validation function with a non-trivial input space. Nectar-owned values come from nectar's `arbitrary` feature: bridge a raw type with `proptest_arbitrary_interop::arb::<T>()` and draw a valid-by-construction value (signed stamp, stamped chunk, signed SOC) by mapping a byte strategy through `arbitrary::Unstructured` into `nectar_primitives::generators`/`nectar_postage::generators`. Enable those features from dev-dependencies only; they must never enter a shipped or wasm cone. The vertex-owned strategies (signed peer records, storage radius) live behind the `proptest` feature of `vertex-swarm-test-utils` (`strategies` module); `vertex-net-codec::prop_assert_proto_roundtrip!` is the round-trip helper for `ProtoMessage` codecs. The reference adopters are `crates/swarm/net/pullsync` (the `ProtoMessage` path via the helper and the generator bridge) and `crates/swarm/net/handshake` (its free-function codec over the peer-record strategy).
