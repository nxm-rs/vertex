...
Four tiers, in order of how much of the stack a test exercises. Reach for the lowest tier that can still fail for the reason you care about.

1. **In-source unit tests.** `#[cfg(test)] mod tests` next to the code, for pure logic and single-type invariants. No swarm, no network, no clock.
2. **Behaviour pair tests on the harness.** `vertex-swarm-test-utils`'s `harness` feature is the standard way to drive two (or more) `NetworkBehaviour`s against each other. Build each side with `seeded_identity`/`seeded_node` so the peer id and overlay are reproducible from one seed byte, connect them with `connect_and_activate` (or `connect_and_activate_hetero` for two different behaviour types), then drive with `drive_until`/`drive_for`/`drive`/`drive_until_signal`. These helpers run their deadline on the tokio clock, so under `#[tokio::test(start_paused = true)]` an idle pair auto-advances instead of burning wall time; this only holds because the vendored libp2p fork now routes every internal timer through the tokio clock on native, so a paused runtime actually pauses the swarm, not just the test's own sleeps. This is the tier for "does this behaviour react correctly to a peer event", one level above pure unit logic.
3. **In-process clusters.** The `cluster` feature (plus `cluster-storer` for storer-role nodes) spins up a `BootNode` and several `ClientNode`/`StorerNode`s sharing one process via `ClusterBuilder`. The default transport is in-process and memory-backed, so it scales to tens of nodes without OS sockets and is the transport to pair with `start_paused` for virtual-time convergence tests. `ClusterBuilder::with_seed` derives every node's signer, nonce, and listen-port base from one seed so a run replays exactly. A TCP transport mode exists for the rare test that needs real sockets; it pays real wall-clock waiting and is not the default. This is the tier for topology convergence, multi-node protocol interplay, and anything that needs more than two peers.
4. **Wire-conformance vectors and property tests.** The `vectors` module (always compiled, no feature gate) is the shared harness for hex vector decoding, byte-diff reporting, and labelled table runs; take it as a plain dev-dependency from any leaf protocol crate. The `proptest` feature of `vertex-swarm-test-utils` adds vertex-owned strategies (signed peer records, storage radius) in its `strategies` module, and `vertex-net-codec::prop_assert_proto_roundtrip!` is the round-trip helper for `ProtoMessage` codecs. Nectar-owned values (signed stamps, stamped chunks, signed SOCs) come through nectar's `arbitrary` feature: bridge a raw type with `proptest_arbitrary_interop::arb::<T>()` and draw a valid-by-construction value by mapping a byte strategy through `arbitrary::Unstructured` into `nectar_primitives::generators`/`nectar_postage::generators`. Enable those features from dev-dependencies only; they must never enter a shipped or wasm cone.

Hand-rolled two-swarm scaffolding (bespoke ephemeral-swarm-plus-connect setup duplicated per test file) and per-crate mock behaviours are deprecated. New tests use the harness and the canonical mocks (`MockIdentity`, `MockTopology`, `MockStorage`, `MockReserve`) in `vertex-swarm-test-utils`; do not reinvent these in a downstream crate's `tests/`.

Seed replay is the debugging workflow tiers 2 and 3 share: a failing seeded test reports the seed it ran with, and rerunning with that same seed reproduces the exact identities and (for a cluster) port allocation that triggered the failure, no bisection needed.

A deterministic simulation tier that drives the same behaviours under virtual time without a real transport is the direction this harness is heading, not yet a shipped tier.
