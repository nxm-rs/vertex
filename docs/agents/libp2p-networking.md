...
Ownership of the live connection mapping is single-writer: topology's connection registry (`PeerRegistry`) is the one place the overlay-to-PeerId map is written, at handshake completion, disconnect, and the stale sweep. Sibling behaviours never keep their own mirror. The client behaviour instead takes a read-only `Arc<dyn ActivePeers<OverlayAddress>>` (from `TopologyBehaviour::identity_view`) and consults it for command routing and for resolving the overlay at `ConnectionClosed`. Two invariants make this safe: the lookup domain is exactly Active (handshake-complete) peers, never pending; and the read at connection close must observe the registry entry before topology removes it, which is guaranteed solely by declaring the client field before `topology` in each node composite so the derived `FromSwarm` fanout reaches the client first. Mirroring the map in a consumer reintroduces the reconnect-race routing loss this design removes (an old connection's close deleting the new mapping).
