...
- Hive (`vertex-swarm-net-hive`): `BroadcastPeers` protobuf shape, peer record signature digest, gossip triggers fire after a successful handshake (peer activation). Per-peer liveness/RTT use stock `libp2p::ping` (`/ipfs/ping`); the reference's `/swarm/pingpong` is an operator-only diagnostic and is not implemented. Ping is a liveness signal only: public reachability (the `ReachabilityTracker` `Public` verdict) is set solely by AutoNAT v2 dial-back or a successful outbound dial to a public address, never by ping or handshake success.
