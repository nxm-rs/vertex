...
In short: dial toward `max(taper, saturation)` and, for peers filling empty sub-prefix slots, up to the retention floor `max(target, oversaturation)`; accept inbound up to `max(target + headroom, oversaturation)`; trim only above the retention floor. Neighborhood bins are unbounded at every level and never produce trim surplus.
...
A peer becomes a candidate only if it passes every check: not already connected, not in a connection phase, not already queued, not banned, not in backoff, not a duplicate within the round, and its bin still admits it counting candidates already selected this round. Admission is `needs_more` (against effective count plus per-bin selections) for a duplicate-slot peer, and the higher `projected < retention_floor` bound for a peer that fills an empty sub-prefix slot.
...
Before a dial is issued, `RoutingCapacity::try_reserve_dial` must succeed: it refuses peers already in a connection phase and bins that no longer need more peers at the current depth. It mirrors balanced selection, so a count-satisfied bin still admits a dial that fills a sub-prefix slot no connected peer covers, up to the bin's retention floor; a duplicate-slot dial past the count target is refused, so the slot spread never becomes a dial storm. The reservation is released on every failure path. A dial failure is additionally recorded against backoff when the attempt actually failed (no reachable addresses, transport-level dial failure, handshake failure, or staleness), but not when dispatch merely skipped the peer (already tracked, or in backoff or banned at dispatch time).
