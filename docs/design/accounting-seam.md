...
`SwarmAccounting::connect_peer(peer, node_type)` is a default-no-op hook the node calls at handshake completion; the real impl re-keys the per-peer creditor serve line on the remote's advertised type from the unscaled base threshold (storer remotes the full base line, client remotes the `client_only_factor`-scaled base line), regardless of our own node type, healing a peer created lazily on the stricter client line in place. Creditor direction only: the debtor-side disconnect line and settle pacing keep the own-type-scaled (`for_client`) values.
