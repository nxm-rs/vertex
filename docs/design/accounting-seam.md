...
`SwarmAccounting::connect_peer(peer, node_type)` is the hook the node calls at handshake completion; the impl re-keys the per-peer creditor serve line on the remote's advertised type from the unscaled base threshold (storer remotes the full base line, client remotes the `client_only_factor`-scaled base line), regardless of our own node type, healing a peer created lazily on the stricter client line in place, and returns the line now in force. The node announces the returned line to the peer once per connection as its initial payment threshold, so the announced value is exactly what the provide gate enforces. Creditor direction only: the debtor-side disconnect line and settle pacing keep the own-type-scaled (`for_client`) values.
