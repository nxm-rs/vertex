...
`SwarmAccounting::connect_peer(peer, node_type)` is a default-no-op hook the node calls at handshake completion; the real impl re-keys the per-peer creditor serve line on the remote's advertised type (storer remotes the full payment threshold, client remotes the `client_only_factor`-scaled line), healing a peer created lazily on the stricter client line in place. Creditor direction only: the debtor-side disconnect line and settle pacing are untouched.
