...
- `Debt` (new): an opaque non-negative newtype (`Debt(u64)`, private field), `Ord`, `From<Debt> for Au` (total). It deliberately has NO `Add`/`Sub`/`Neg` against `Au` and NO `From<Au>`: a signed balance can never be fed in, so the sign error that compared a negative balance to a positive threshold is unrepresentable. Three named constructors, because admission and settlement measure different things:
  ...
  - `Debt::project(balance, reserved, price) = max(0, reserved + price - balance)`: what we would owe including the pending reservation and this request. The RECEIVE admission gate uses this.
  - `Debt::exposure(balance, shadow_reserved, ghost, price) = max(0, balance + shadow_reserved + ghost + price)`: what the peer owes us once this provide commits. The PROVIDE gate uses this, mirroring the receive gate: `exceeds(payment_threshold)` refuses. A negative balance widens serve headroom; ghost debt counts so a repeat refuser starves.
...
`SwarmAccounting::connect_peer(peer, node_type)` is the hook the node calls at handshake completion; the impl re-keys the per-peer creditor serve line and the per-peer settlement allowance rate on the remote's advertised type from the unscaled bases (storer remotes the full base values, client remotes the `client_only_factor`-scaled ones), regardless of our own node type, healing a peer created lazily on the stricter client values in place, and returns the serve line now in force. The node announces the returned line to the peer once per connection as its initial payment threshold, so the announced value is exactly what the provide gate enforces. Creditor direction only: the debtor-side disconnect line and settle pacing keep the own-type-scaled (`for_client`) values.

Inbound settlement lands through one seam: `SwarmPeerAccounting::settlement_received(amount)` credits the ledger, accumulates the peer's cumulative repayment on `PeerState`, and evaluates the threshold-growth checkpoint in the same step, returning a `SettlementCredit` carrying the running total and, when a checkpoint was crossed, the raised serve line (one refresh-rate step, linear `rate * 100` checkpoints to `rate * 1800` then doubling, at most one raise per event). Every settlement method's validated inbound credit calls it (pseudosettle today, swap cheques when they land), so no second per-peer repayment map or growth schedule can appear outside accounting; validation stays with the caller that owns the pacing clocks, and the caller's only growth duty is turning the raised line into a threshold re-announcement on the pricing protocol. `connect_peer` resets the accumulator and checkpoint with the serve line, so growth is earned per connection.
