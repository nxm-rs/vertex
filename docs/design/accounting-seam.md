...
Inbound settlement lands through one seam: `SwarmPeerAccounting::settlement_received(amount)` credits the ledger and accumulates the peer's cumulative repayment on `PeerState`, returning the running total that threshold growth checkpoints against. Every settlement method's validated inbound credit calls it (pseudosettle today, swap cheques when they land), so no second per-peer repayment map can appear outside accounting; validation stays with the caller that owns the pacing clocks.
