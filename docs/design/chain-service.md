...
With the feature, the launch path for a chain-needing node (a storer always, a client only when SWAP settlement is enabled, decided by `SwarmNodeType::needs_chain`) confirms the spec resolves a canonical contract deployment (Gnosis for mainnet, Sepolia for testnet) so a network with none fails fast, takes the validation chain from `spec.chain` and the signer from the node's Ethereum identity, then calls `build_chain_provider`. The constructor builds a wallet-filled provider over the configured RPC URL signed by that identity, and validates the connected chain id against the expected chain so an operator pointed at the wrong endpoint fails fast at startup. The result is a cloneable `SharedChainProvider` handle that carries only the live connection: the validation chain and the contract addresses are spec-derived, so each consumer reads its own `ChainConfig` from the spec at the edge rather than from the handle. There is no background chain task to spawn: the provider is the chain, and consumers borrow it.
