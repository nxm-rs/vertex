...
use beers_cli_runner::CliRunner;
// use reth_db::DatabaseEnv;
// use reth_node_builder::{NodeBuilder, WithLaunchContext};
// use reth_primitives::ChainSpec;
use beers_tracing::FileWorkerGuard;
...
#[derive(Debug, Parser)]
#[command(author, version = SHORT_VERSION, long_version = LONG_VERSION, about = "Beers", long_about = None)]
pub struct Cli {
    /// The command to run
    #[command(subcommand)]
    command: Commands,
...
impl Cli {
    ...
    where
    {
        ...
        // self.logs.log_file_directory = "beers/logs/".to_string();
            // self.logs.log_file_directory.join(self.chain.chain.to_string());

        let _guard = self.init_tracing()?;
        info!(target: "beers::cli", "Initialized tracing, debug log directory: {}", "beers/logs/".to_string());
