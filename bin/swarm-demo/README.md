...
- The address book persists in a separate IndexedDB database (`vertex-swarm-peers`), an IndexedDB-backed peer snapshot store supplied through `with_peer_store`, so the node re-converges warm across reloads. A failed open falls back to the runtime-only peer set, the same pattern as the cache.
