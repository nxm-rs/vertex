...
pub struct LogArgs {
    ...
    /// TODO: Potentially look at changing this to a `PlatformPath` to allow for platform-specific
    ...
    /// Write telemetry logs to OTLP
    #[arg(long = "log.telemetry", global = true)]
    pub telemetry: bool,

    /// Which endpoint to send telemetry logs to.
    /// This is required if telemetry is enabled.
    #[arg(long = "log.telemetry.endpoint", value_name = "ENDPOINT", global = true)]
...
impl LogArgs {
    ...
    pub fn init_tracing(&self) -> eyre::Result<Option<FileWorkerGuard>> {
        let mut tracer = BeersTracer::new();
        ...
        if self.telemetry {
            // tracer = tracer.with_telemetry(stdout, self.telemetry_endpoint.unwrap())
        }
