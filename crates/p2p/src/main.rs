...
use alloy::{hex, primitives::FixedBytes, signers::{wallet::LocalWallet, Signature}};
use anyhow::Result;
use futures::{AsyncReadExt, AsyncWriteExt, StreamExt};
use handshake::swarm::handshake::BzzAddress;
use libp2p::{
    autonat, identify, identity, multiaddr::Protocol, noise, ping, swarm::{NetworkBehaviour, SwarmEvent}, tcp, yamux, Multiaddr, PeerId, Stream, StreamProtocol
};
use libp2p_stream as stream;
use rand::RngCore;
use swarm::NodeAddress;
...
async fn main() -> Result<()> {
    ...
    let peer = "/ip4/127.0.0.1/tcp/1634/p2p/QmcmeGcwx3YTBmBTQ5MdGh1SJfCpue7v5fqVuE8iUv9JYb".parse::<Multiaddr>()?;

    swarm.dial(peer.clone())?;

    tokio::spawn(connection_handler(peer.clone(), swarm.behaviour().handshake.new_control(), "/ip4/0.0.0.0/tcp/2634".parse()?));
...
async fn connection_handler(ma: Multiaddr, mut control: stream::Control, own_underlay: Multiaddr) -> Result<()> {
    let Protocol::P2p(peer_id) = ma.clone().pop().expect("peer address must have a p2p component") else {
        return Err(anyhow::anyhow!("peer address must have a p2p component"));
    };
...
async fn send(mut stream: Stream, ma: Multiaddr, own_underlay: Multiaddr) -> io::Result<()> {
    ...
    stream.write_all(&syn.encode_length_delimited_to_vec()).await?;
    stream.flush().await?;
    ...
    println!("{:?}", recv_amt);
    let syn_ack = handshake::swarm::handshake::SynAck::decode_length_delimited(b.as_slice()).unwrap();
    ...
    stream.write_all(&ack.encode_length_delimited_to_vec()).await?;
    stream.flush().await?;
