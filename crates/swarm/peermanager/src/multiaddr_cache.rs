//! Internal multiaddr registry for PeerId ↔ OverlayAddress mapping.
//!
...
pub(crate) struct MultiAddrRegistry {
...
/// Default maximum number of entries in the multiaddr cache.
pub const DEFAULT_CACHE_MAX_SIZE: usize = 10_000;

/// Default TTL for cache entries (1 hour).
pub const DEFAULT_CACHE_TTL: Duration = Duration::from_secs(3600);

/// Configuration for the multiaddr cache.
#[derive(Debug, Clone)]
pub(crate) struct MultiAddrCacheConfig {
...
impl Default for MultiAddrCacheConfig {
    fn default() -> Self {
        Self {
...
/// A cached multiaddr entry with expiration time.
#[derive(Debug, Clone)]
struct CacheEntry {
    /// The cached multiaddr addresses.
...
impl CacheEntry {
    fn new(multiaddrs: Vec<Multiaddr>, ttl: Duration) -> Self {
        Self {
            multiaddrs,
...
/// Cache of known multiaddr addresses (Multiaddr) for overlay addresses.
///
...
pub(crate) struct MultiAddrCache {
...
impl std::fmt::Debug for MultiAddrCache {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MultiAddrCache")
...
impl Default for MultiAddrCache {
    fn default() -> Self {
        Self::new(MultiAddrCacheConfig::default())
    }
}
...
impl MultiAddrCache {
    ...
    /// Cache multiaddrs for an overlay address.
    ///
    /// If multiaddrs already exist for this overlay, they are replaced.
    /// LRU eviction happens automatically when capacity is exceeded.
    pub fn insert(&mut self, overlay: OverlayAddress, multiaddrs: Vec<Multiaddr>) {
        self.cache
            .put(overlay, CacheEntry::new(multiaddrs, self.ttl));
    }

    /// Get cached multiaddrs for an overlay address.
    ///
    ...
    pub fn get(&mut self, overlay: &OverlayAddress) -> Option<&Vec<Multiaddr>> {
        ...
        self.cache.get(overlay).map(|e| &e.multiaddrs)
    }

    /// Get cached multiaddrs without updating LRU order (for batch filtering).
    ///
    ...
    pub fn peek(&self, overlay: &OverlayAddress) -> Option<&Vec<Multiaddr>> {
        ...
        Some(&entry.multiaddrs)
    }

    /// Remove cached multiaddrs for an overlay address.
    pub fn remove(&mut self, overlay: &OverlayAddress) -> Option<Vec<Multiaddr>> {
        self.cache.pop(overlay).map(|e| e.multiaddrs)
    }

    /// Check if we have valid (non-expired) cached multiaddrs for an overlay.
    ...
    ///
    /// Returns overlay addresses and their multiaddrs.
    pub fn entries_for_persistence(&self) -> Vec<(OverlayAddress, Vec<Multiaddr>)> {
        self.cache
            .iter()
            .filter(|(_, entry)| !entry.is_expired())
            .map(|(overlay, entry)| (*overlay, entry.multiaddrs.clone()))
            .collect()
    }
    ...
    pub fn load_from_persistence(
        ...
        for (overlay, multiaddrs) in entries {
            if !multiaddrs.is_empty() {
                self.insert(overlay, multiaddrs);
            }
        }
    }
}
...
mod tests {
    ...
    fn test_registry_basic() {
        let mut registry = MultiAddrRegistry::new();
    ...
    #[test]
    fn test_cache_basic() {
        let config = MultiAddrCacheConfig::default();
        let mut cache = MultiAddrCache::new(config);
    ...
    fn test_cache_lru_eviction() {
        ...
        let config = MultiAddrCacheConfig::default().with_max_size(3);
        let mut cache = MultiAddrCache::new(config);
    ...
    fn test_cache_ttl_expiration() {
        ...
        let config = MultiAddrCacheConfig::default().with_ttl(Duration::from_millis(10));
        let mut cache = MultiAddrCache::new(config);
    ...
    fn test_cache_peek_vs_get() {
        let config = MultiAddrCacheConfig::default().with_max_size(2);
        let mut cache = MultiAddrCache::new(config);
    ...
    fn test_cache_persistence_roundtrip() {
        let config = MultiAddrCacheConfig::default();
        let mut cache = MultiAddrCache::new(config.clone());
        ...
        let mut cache2 = MultiAddrCache::new(config);
