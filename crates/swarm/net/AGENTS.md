# AGENTS: crates/swarm/net/

The `/swarm/...` wire protocols. Each crate defines exactly one libp2p protocol: behaviour, connection handler, codec, error, metrics. `proto` holds every generated protobuf module, so a protocol crate depends only on `vertex-swarm-net-proto`, never on a build script of its own.

Global rules: see root `/AGENTS.md`. Deep guides for changes here: `docs/agents/swarm-protocol.md` (wire conformance, fork gating) and `docs/agents/libp2p-networking.md`.

## Crates and protocol IDs

- `handshake`: `/swarm/handshake/15.0.0/handshake`. Identity exchange and admission control. Non-headered.
- `hive`: signed peer-record gossip for topology bootstrap.
- `pricing`: `/swarm/pricing/1.0.0/pricing`. Payment threshold announcement.
- `pseudosettle`: `/swarm/pseudosettle/1.0.0/pseudosettle`. Bandwidth micro-payments.
- `pullsync`: `/swarm/pullsync/1.4.0/cursors` (Syn/Ack cursor handshake) and `/swarm/pullsync/1.4.0/pullsync` (Get/Offer/Want/Delivery range exchange). One protocol, two streams: the "one `PROTOCOL_NAME` per crate" rule bends here, exposed as `PROTOCOL_CURSORS` and `PROTOCOL_SYNC`.
- `pushsync`: `/swarm/pushsync/1.3.1/pushsync`. Chunk push with receipts.
- `retrieval`: `/swarm/retrieval/1.4.0/retrieval`. Chunk request and delivery.
- `headers`: shared header frame for request-response protocols, with trace-context propagation. W3C-over-OpenTelemetry inject/extract is native-only (`tracing.rs`); the wasm sibling (`tracing_wasm.rs`, Pattern C) is a no-op since a browser client has no OTLP backend. The on-wire `tracing-span-context` field is unaffected.
- `handler-core`: handler scaffolding. `HandlerCore<E>` (unbounded pending events, GCRA rate limiter, outbound-pending flag) plus two opt-in blocks composed a la carte: `BoundedQueue<T>` (bounded FIFO with a caller-owned reject-newest or evict-oldest policy) and `OutcomeDriver<O>` (a capped `FuturesUnordered` outcome driver). The blocks carry mechanics only; the drop policy, metrics, warn lines, and cap constants stay at the call site.
- `identify`: vendored libp2p-identify with a targeted-push extension and the shared per-connection observed-address registry (`ObservedAddresses`, entries dying with their connection), the source for the topology handle's read-only observed external address vote.
- `proto`: consolidated protobuf modules. Re-exports `handshake`, `headers`, `hive`, `pricing`, `pseudosettle`, `pullsync`, `pushsync`, `retrieval`, `swap`.

## Dos

- One protocol per crate, one `PROTOCOL_NAME` constant. Reference that constant from tests and metrics labels.
- Implement the codec in its own `codec` module, separate from the behaviour. Wire types live behind a domain wrapper so the protobuf type never escapes.
- Compose `vertex-net-codec::FramedProto` for framing. Use its `protocol_error!` macro for the common error variants (`ConnectionClosed`, `Protobuf`, `Io`).
- Compose `HandlerCore` for the rate-limited inbound queue and outbound flag. Reach for `BoundedQueue` when a handler needs a bounded pending-event or pending-command queue with an explicit drop policy, and `OutcomeDriver` when it drives a capped set of inbound or outbound outcome futures. Keep the policy (reject-newest vs evict-oldest), the metric name, the warn line, and the cap constant at the call site; the blocks are pieces, not a framework.
- Embed metrics in a `pub mod metrics` submodule and derive `strum::IntoStaticStr` on event/stage enums so labels are static strs.
- Pull the protobuf module from `vertex-swarm-net-proto`. Never add a `build.rs` or `OUT_DIR` include path in a protocol crate.

## Donts

- Do not inline the protobuf module into the behaviour file. Generated code lives in `vertex-swarm-net-proto`, consumed via `pub use`.
- Do not let the protobuf type escape the codec boundary. The behaviour deals in domain types.
- Do not depend on `vertex-swarm-storer`, `vertex-swarm-localstore`, or `vertex-swarm-topology` from a protocol crate. Expose traits and let higher layers wire them.
- Do not assume a single connection per peer in handler state. `KeyedRateLimiter` exists so behaviour-level limits work across handlers.
- Do not `unwrap`/`expect` on protobuf decoding paths. The workspace lints them as warnings on purpose.

## Wire conformance

Pair every wire-visible change with a conformance vector update under the protocol crate's `tests/`; `crates/swarm/peers/peer/tests/interop.rs` is the model. Wire changes that diverge from the reference must be gated behind a `SwarmHardfork` and selected via `ForkDigest` (see root `/AGENTS.md` and `docs/agents/swarm-protocol.md`).

## Tests and interop

- `cargo test -p vertex-swarm-net-<name>` for unit tests.
- `handshake` carries cross-implementation wire-conformance vectors. Run `cargo test -p vertex-swarm-net-handshake` after any wire change and update the vector files if the change is intentional.
- `libp2p-swarm-test` is the harness for behaviour-level tests.
- The pushsync codec has libFuzzer targets under `fuzz/` at the repo root; its committed seeds replay on stable via `seed_replay_pushsync_decode` in the codec tests. Read `fuzz/README.md` before adding a target for another protocol crate.
- The handshake codec has `handshake_decode`, `handshake_roundtrip`, and `handshake_uvarint_differential` targets; the fuzz-facing surface (decode entry points, the frame-sizing arithmetic, and valid-by-construction generators) lives in the crate's `fuzz` module behind the `arbitrary` feature, and the committed seeds replay via `seed_replay_handshake_decode` in `codec/mod.rs`. The decode target drives adversarial field content through the domain decode; the raw generated protobuf reader is upstream (a nested-length overflow in `quick-protobuf` under `overflow-checks` is tracked out of band).
- The hive batch-validation path has the `hive_decode` target and `identify`'s message conversions have `identify_decode`; each crate carries a `fuzz` module behind its `arbitrary` feature (hive exposes the batch validation entry and routable signed-record generators, identify a byte-level decode plus a message generator), with seeds replayed via `seed_replay_hive_decode` in `hive/src/protocol.rs` and `seed_replay_identify_decode` in `identify/src/protocol.rs`. The `hive_decode` oracle also pins the amplification bound: full-validation successes stay proportional to wire bytes, so the frame cap bounds per-message ECDSA work.
- The pullsync codec has `pullsync_decode`, `pullsync_bitvector`, and `pullsync_roundtrip` targets. The flat frames take raw bytes; the nested `Offer` goes through the write then read path via `fuzz::arbitrary_wire_offer`, and `fuzz::check_bitvector` carries the bitvector invariant sweep (the `len / 8 + 1` byte-length rule, pad bits staying invisible, the `into_bytes`/`from_bytes` round-trip) so the target and the stable replay share one path. The `Arbitrary` impls behind the `arbitrary` feature also drive the crate's proptest suite; seeds replay via `seed_replay_pullsync_decode` in `pullsync/src/codec.rs` and `seed_replay_pullsync_bitvector` in `pullsync/src/bitvector.rs`.
- The accounting codecs have `pricing_decode`/`pricing_roundtrip`, `pseudosettle_decode`/`pseudosettle_roundtrip`, and `swap_decode`/`swap_roundtrip` targets: these frames carry peer-controlled monetary quantities, so the decode oracles are stronger than "no panic". Each crate's `fuzz` module (behind its `arbitrary` feature) exposes the raw decode entries; the pricing and pseudosettle entries assert a decoded amount re-encodes to the identical wire bytes (the canonical trimmed big-endian decode, oversized and leading-zero inputs erroring), and the swap cheque entry asserts stability across re-encode (the JSON payload is value-preserving, not byte-canonical). The `Arbitrary` impls also drive each crate's proptest suite; seeds replay via `seed_replay_pricing_decode`, `seed_replay_pseudosettle_decode`, and `seed_replay_swap_decode` in the respective `codec.rs`. Swap is a gated capability: its `arbitrary` surface is consumed only by the fuzz workspace, never by the default or wasm client cones. The shared `U256` wire helpers carry their own `u256_wire` property target in `vertex-net-codec` (see `crates/net/AGENTS.md`).
- The retrieval codec has `retrieval_decode` and `retrieval_roundtrip` targets and the headers envelope has `headers_decode` and `headers_roundtrip`. Retrieval's `fuzz` module (behind `arbitrary`) exposes the raw decode entries (a delivery fuzz input is the 32-byte requested address followed by the raw frame, so the fuzzer controls both sides of the chunk reconstruction) and `fuzz::check_delivery_roundtrip`, the address-parameterized round-trip whose oracle is the stampless projection (the serve path ships chunk data only). Headers' `fuzz` module exposes `decode_headers`, an adversarial envelope generator with deliberately repeated keys, and `check_headers`, which pins the duplicate-key collapse to the last occurrence and re-encode stability. The `Arbitrary` impls behind each crate's `arbitrary` feature also drive the proptest suites; seeds replay via `seed_replay_retrieval_decode` in `retrieval/src/codec.rs` and `seed_replay_headers_decode` in `headers/src/codec.rs`.
