...
- `localstore`: the `SwarmLocalStore` client cache (`ChunkStore`, a thin wrapper over the byte-bounded LRU in `vertex-store`) plus its config. Content chunks are served indefinitely; single-owner chunks are served only while their stamp timestamp is within `soc_cache_ttl`, and a newer SOC wins on insert. The persisting storer reserve implements the same `SwarmLocalStore` trait separately and is tracked apart from the cache-only client.
