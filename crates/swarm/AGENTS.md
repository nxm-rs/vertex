...
- `node`: composes the libp2p `NetworkBehaviour` and exposes `BootNode`, `ClientNode`, `StorerNode`. This is where libp2p shows up. Also owns `PeerSelector`: retrieval and pushsync candidate selection ordered by peer score and affordability on top of proximity. The builder wires it from the topology handle (scores via the peer manager) and the bandwidth accounting (affordability, per-peer chunk price, best-effort settlement trigger). Also owns `SelfThrottle`: the outbound self-throttle (`throttle.rs`) that paces our own retrieval and pushsync requests under each peer's pseudosettle allowance before `ClientHandle::retrieve_chunk`/`push_chunk` dispatches. One throttle is shared by both protocols; its allowance signal is the same `PeerAffordability` the selector reads (built once in accounting, never hard-coded), its tokens are settle units of `refresh_rate` AU per second, and the per-peer bucket is resized on every allowance change and cleared on disconnect. Tripping it increments `pushsync_self_throttled_total{peer_overlay}` / `retrieval_self_throttled_total{peer_overlay}`.
