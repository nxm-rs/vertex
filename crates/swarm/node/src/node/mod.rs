...
//!
//! ```ignore
//! use vertex_swarm_node::{SwarmNode, NetworkConfig};
...
/// - `N: SwarmNodeTypes` - base network participation (bootnode)
/// - `N: SwarmClientNodeTypes` - can retrieve and upload chunks (client)
...
impl<N: SwarmNodeTypes> SwarmNode<N> {
    ...
    ///
    /// Combines `start_listening()`, `connect_bootnodes()`, and `run()`.
    ...
    /// Prefer spawning via `SpawnableTask::into_task()` which handles setup.
    ...
    fn handle_topology_event(&mut self, event: TopologyEvent) {
        match event {
            ...
            TopologyEvent::HivePeersReceived { from, peers } => {
                ...
                // Extract overlays and cache multiaddrs for dialing
                ...
                // Cache multiaddrs synchronously (needed for dialing)
    ...
    fn dial_connection_candidates(&mut self) {
        ...
        // Batch filter: get dialable candidates with cached multiaddrs (single lock acquisition)
...
pub struct SwarmNodeBuilder<N: SwarmNodeTypes> {
    ...
    /// Optional sender for routing pseudosettle events to a service.
    pseudosettle_event_tx: Option<mpsc::UnboundedSender<PseudosettleEvent>>,
    /// Optional sender for routing swap events to a service.
    swap_event_tx: Option<mpsc::UnboundedSender<SwapEvent>>,
    /// NAT addresses to advertise.
    nat_addrs: Vec<Multiaddr>,
    /// Enable auto NAT discovery from observed addresses.
    nat_auto: bool,
}
...
impl<N: SwarmNodeTypes> SwarmNodeBuilder<N> {
    ...
    /// Set the sender for routing pseudosettle events to a service.
    ///
    /// When set, the behaviour will route pseudosettle events to this channel
    /// in addition to the main client event channel.
    ...
    /// Set the sender for routing swap events to a service.
    ///
    /// When set, the behaviour will route swap events to this channel
    /// in addition to the main client event channel.
    ...
    pub async fn build(self) -> Result<(SwarmNode<N>, ClientService, ClientHandle)> {
        ...
        // Create AddressManager (always enabled for listen address tracking)
        let address_manager = {
        ...
        let address_manager_for_behaviour = address_manager.clone();
