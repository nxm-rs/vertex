...
pub fn record_chunk_transfer(amount_au: u64, upload: bool) {
    ...
    metrics::counter!("accounting_chunks_total", "direction" => dir).increment(1);
    metrics::counter!("accounting_au_total", "direction" => dir).increment(amount_au);
}
...
pub fn record_settlement_attempt(provider: &'static str) {
    metrics::counter!("accounting_settlement_attempts_total", "provider" => provider).increment(1);
}
...
pub fn record_settlement_success(provider: &'static str, amount: i64) {
    metrics::counter!("accounting_settlement_success_total", "provider" => provider).increment(1);
    if amount > 0 {
...
/// Record that the trust ramp grew the local credit limit for a peer.
