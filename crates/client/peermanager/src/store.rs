...
/// the full BzzAddress (overlay, underlays, signature, nonce) along with
...
/// ```json
/// {
///   "0x1234...": { "overlay": "0x1234...", "underlays": [...], ... },
///   "0x5678...": { "overlay": "0x5678...", "underlays": [...], ... }
/// }
/// ```
///
/// # Thread Safety
...
mod tests {
    ...
    fn test_memory_store_update() {
        ...
        assert_eq!(store.count().unwrap(), 1);
        let loaded = store.get(&peer.overlay).unwrap().unwrap();
        assert_eq!(loaded.connection_count, 10);
    }
    ...
    fn test_file_store_update() {
        ...
        assert_eq!(loaded.connection_count, 42);
    }
