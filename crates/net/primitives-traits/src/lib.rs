...
pub trait NodeAddress {
    ...
    fn overlay_address(&self) -> SwarmAddress {
        calculate_overlay_address(self.chain_address(), self.network_id(), self.nonce())
    }

    /// Returns the network ID this address belongs to.
...
/// Calculates the overlay address for a node given its chain address, network ID, and nonce.
///
...
/// * `network_id` - The network ID (1 for mainnet, 10 for testnet, etc.)
/// * `nonce` - A unique nonce value
pub fn calculate_overlay_address(
