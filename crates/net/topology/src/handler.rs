...
pub enum Event {
    ...
    HivePeersReceived(Vec<BzzAddress>),
...
pub struct TopologyHandler<N: SwarmNodeTypes> {
    ...
    pending_hive_outbound: VecDeque<Peers>,
    ...
    fn handle_inbound_output(&mut self, output: TopologyInboundOutput) {
        match output {
            TopologyInboundOutput::Handshake(info) => {
                debug!(peer_id = %self.peer_id, overlay = %info.ack.node_address().overlay_address(), "Handshake completed (inbound)");
                self.state = State::Ready;
                self.pending_events
                    .push_back(Event::HandshakeCompleted(info));
            }
            TopologyInboundOutput::Hive(peers) => {
                debug!(peer_id = %self.peer_id, count = peers.peers.len(), "Received hive peers");
                let addresses: Vec<BzzAddress> = peers.peers;
                self.pending_events
                    .push_back(Event::HivePeersReceived(addresses));
            }
    ...
    fn handle_outbound_output(
        ...
        match (output, info) {
            (
                ...
                self.handshake_outbound_pending = false;
                debug!(peer_id = %self.peer_id, overlay = %handshake_info.ack.node_address().overlay_address(), "Handshake completed (outbound)");
                self.state = State::Ready;
                self.pending_events
            ...
            (output, info) => {
                warn!(peer_id = %self.peer_id, ?output, ?info, "Mismatched outbound output and info");
            }
        }
    }
}
