...
Bee uses a custom `0x99` prefix for encoding multiple multiaddrs in a single bytes field. This is **not standard libp2p** - standard approach uses `repeated bytes` in protobuf.

Located in `vertex-net-primitives`: `serialize_underlays()` / `deserialize_underlays()`
