...
- `peer/backoff`, `peer/score`, `peer/store`, `peer/registry`: peer state primitives with no protocol logic. `peer/registry` also exports the `ActivePeers<Id>` read-view trait (`active_peer_id`/`active_id`, Active-state only, distinct from the pending-inclusive `resolve_*`) so a consumer holding an `Arc<dyn ActivePeers<Id>>` looks up the handshake-complete identity map without mirroring it or writing to the registry. With `local` they sit in the wasm compilation cone of the Swarm peer stack: keep them building for `wasm32-unknown-unknown` (the `wasm` CI job enforces this through `vertex-swarm-peer-manager`) and take wall and monotonic clocks from `web-time`, not `std::time`. See `docs/agents/wasm.md`.
- `dialer`: generic dial-request tracker with bounded queue and in-flight management.
- `codec`: protobuf framing (`FramedProto`), the `protocol_error!` macro for protocol-shaped error enums, and the trimmed big-endian `U256` wire helpers (`encode_u256_be`/`decode_u256_be`). The decode is strict canonical (empty is zero; a leading zero byte or more than 32 bytes is a `U256DecodeError`, never a trim, truncate, or panic) because every accounting codec funnels peer-controlled monetary quantities through it. The pair carries the `u256_wire` fuzz target under `fuzz/` at the repo root; the frame layer (the `Codec` wrapper's cap wiring and error mapping plus `FramedProto`) carries the `framing_decode` target. Both invariant sweeps live in the crate's `fuzz` module behind the `arbitrary` feature, with committed seeds replayed on stable via `seed_replay_u256_wire` in `utils.rs` and `seed_replay_framing_decode` in `framed.rs`.
