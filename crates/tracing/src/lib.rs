...
//!  ```
//!  use beers_tracing::{
//!      LayerInfo, BeeTracer, Tracer,
//!      tracing::level_filters::LevelFilter,
//!      LogFormat,
//!  };
//!
//!  fn main() -> eyre::Result<()> {
//!      let tracer = BeeTracer::new().with_stdout(LayerInfo::new(
//!          LogFormat::Json,
...
#![doc(
    issue_tracker_base_url = "https://github.com/rndlabs/bee-rs/issues/"
)]
...
#[derive(Debug, Clone)]
pub struct BeersTracer {
    stdout: LayerInfo,
    journald: Option<String>,
    file: Option<(LayerInfo, FileInfo)>,
    telemetry: Option<(LayerInfo, String)>,
}
...
impl BeersTracer {
    ...
    ///  Journald, file layers, and telemetry are not set by default.
    pub fn new() -> Self {
        Self { stdout: LayerInfo::default(), journald: None, file: None, telemetry: None }
    }
    ...
    ///  Sets the telemetry layer configuration and associated endpoint.
    /// 
    /// # Arguments
    /// * `config` - The `LayerInfo` to use for the telemetry layer.
    /// * `endpoint` - The endpoint for the telemetry layer.
    pub fn with_telemetry(mut self, config: LayerInfo, endpoint: String) -> Self {
        self.telemetry = Some((config, endpoint));
        self
    }
}

impl Default for BeersTracer {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Tracer for BeersTracer {
    ...
    fn init(self) -> eyre::Result<Option<WorkerGuard>> {
        ...
        if let Some((config, endpoint)) = self.telemetry {
            layers.telemetry(&config.filters, endpoint)?;
        }
