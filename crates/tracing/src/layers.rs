...
const BEERS_LOG_FILE_NAME: &str = "beers.log";
...
impl Layers {
    ...
    pub(crate) fn telemetry(&mut self, filter: &str, endpoint: String) -> eyre::Result<()> {
        let filter = build_env_filter(None, filter)?;
...
impl FileInfo {
    ...
    pub fn new(dir: PathBuf, max_size_bytes: u64, max_files: usize) -> Self {
        Self { dir, file_name: BEERS_LOG_FILE_NAME.to_string(), max_size_bytes, max_files }
    }
