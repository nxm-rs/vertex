...
use beers_tasks::{TaskExecutor, TaskManager};
...
impl CliRunner {
    ...
    where
        ...
        E: Send + Sync + From<std::io::Error> + From<beers_tasks::PanickedTaskError> + 'static,
    {
        ...
        if command_res.is_err() {
            error!(target: "beers::cli", "shutting down due to error");
        } else {
            debug!(target: "beers::cli", "shutting down gracefully");
        ...
        let _ = rx.recv_timeout(Duration::from_secs(5)).inspect_err(|err| {
            debug!(target: "beers::cli", %err, "tokio runtime shutdown timed out");
        });
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<beers_tasks::PanickedTaskError> + 'static,
{
    {
        let fut = pin!(fut);
        tokio::select! {
    ...
    E: Send + Sync + 'static + From<std::io::Error>,
{
    ...
    #[cfg(unix)]
    {
        ...
        tokio::select! {
            _ = ctrl_c => {
                trace!(target: "beers::cli", "Received ctrl-c");
            },
            _ = sigterm => {
                trace!(target: "beers::cli", "Received SIGTERM");
            },
            res = fut => res?,
        }
    }
